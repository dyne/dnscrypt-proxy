...
After having downloaded a file, compute its SHA256 digest. For example:
    ...
    $ openssl dgst -sha256 dnscrypt-proxy-1.8.1.tar.bz2

Verify this digest against the expected one, that can be retrieved
using a simple DNS query:

    $ drill -aD TXT dnscrypt-proxy-1.8.1.tar.bz2.download.dnscrypt.org

or

    $ dig +dnssec TXT dnscrypt-proxy-1.8.1.tar.bz2.download.dnscrypt.org
