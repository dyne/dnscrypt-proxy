...
## OPTIONS
  ...
  * `-t`, `--test=<grace period>`: don't actually start the proxy, but check that
    a valid certificate can be retrieved from the server and that it
    will remain valid for the next <grace period> minutes. The exit code is 0
    ...
    going to expire before <grace period>. The grace period is specificied in
    minutes.
